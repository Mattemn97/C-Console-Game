...
char getCursorChar()
{
    ...
    {
        DWORD read = 0;
        if (!ReadConsoleOutputCharacterA(hcon,&c,1,con.dwCursorPosition,&read) || read != 1)	c = '\0';
    }
    return c;
}
...
void tworzArmatePoz(int lp, int x,int y, char pagina,int szybkosc)
{
    armataPoz[lp].x=x;
    armataPoz[lp].y=y;
    armataPoz[lp].pagina=pagina;
...
void tworzArmatePio(int lp, int x,int y, char pagina,int szybkosc)
{
    armataPio[lp].x=x;
    armataPio[lp].y=y;
    armataPio[lp].pagina=pagina;
...
void tworzRandomer(int lp,int x,int y, int vite,int zakres,int szybkosc)
{
    randomer[lp].x=x;
    randomer[lp].y=y;
...
void tworzBiegaczaPoz(int lp,int x,int y,char pagina,int vite)
{
    biegaczPoz[lp].x=x;
    biegaczPoz[lp].y=y;
    biegaczPoz[lp].vite=vite;
    biegaczPoz[lp].pagina=pagina;
    gotoxy(x,y);
...
void tworzZbierajkemunizioni(int lp,int x,int y)
{
    zbierajka[lp].istnieje=true;
    zbierajka[lp].x=x;
    zbierajka[lp].y=y;
...
void tworzBossa()
{
    ...
    for (int i=0; i<=7;i++)
        {
            tworzZbierajkemunizioni(12+i,60+i,36);
            tworzZbierajkemunizioni(19+i,78+i,36);
        }
}
...
//---------------------------------------------------------------
        ...
        {
            tworzZbierajkemunizioni(1,15,36);
        }
        if (zbierajka[2].istnieje==true)
        {
            tworzZbierajkemunizioni(2,8,1);
        }
...
int kolizjaRandomera(int lp,char pagina)
{
    if(pagina=='w')
    {
...
void aktualizuj()
{
    if ((giocatore.checkPoint==4) && (scigaczePostawieni==false))
    {
        tworzScigacza(1,42,9);
        tworzScigacza(2,45,13);
        tworzScigacza(3,42,4);
    ...
    if ((giocatore.checkPoint==5)&& (bossPostawiony==false))
    {
        tworzBossa1Raz();
        bossPostawiony=true;
    }
    if ((c=='w')|| (c=='s') || (c=='a') || (c=='d'))
    {
        if (c=='w')
        {
            ...
            cout<<" ";
            if ((readChar(giocatore.x,giocatore.y-1)!= '#') && (readChar(giocatore.x,giocatore.y-1)!= '-') && (readChar(giocatore.x,giocatore.y-1)!= '|')&& (readChar(giocatore.x,giocatore.y-1)!= 'X'))
            {
                if (readChar(giocatore.x,giocatore.y-1)=='@')
                {
                    if ((readChar(giocatore.x,giocatore.y-2)==' ') || (readChar(giocatore.x,giocatore.y-2)=='o'))
                    {
                        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),8);
                        gotoxy(giocatore.x,giocatore.y-2);
                        cout<<"@";
                        giocatore.y-=1;
                    }
                }else
        ...
        if (c=='a')
        {
            ...
            gotoxy(giocatore.x,giocatore.y);
            cout<<" ";
            if ((readChar(giocatore.x-1,giocatore.y)!= '#') && (readChar(giocatore.x-1,giocatore.y)!= '-') && (readChar(giocatore.x-1,giocatore.y)!= '|')&& (readChar(giocatore.x-1,giocatore.y)!= 'X'))
            {
                if (readChar(giocatore.x-1,giocatore.y)=='@')
                {
                    if ((readChar(giocatore.x-2,giocatore.y)==' ') || (readChar(giocatore.x-2,giocatore.y)=='o'))
                    {
                        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),8);
                        gotoxy(giocatore.x-2,giocatore.y);
                        cout<<"@";
                        giocatore.x-=1;
                    }
                }else
        ...
        if (readChar(giocatore.x,giocatore.y)== 'A')
        {
            giocatore.munizioni+=5;
            for(int i=0; i<=20; i++)
            {
                if ((giocatore.x==zbierajka[i].x) && (giocatore.y==zbierajka[i].y))
                {
                    zbierajka[i].istnieje=false;
                }
            }
        }
        if (readChar(giocatore.x,giocatore.y)== '+')
        {
            giocatore.vite+=1;
            for(int i=0; i<=20; i++)
            {
        ...
        if (readChar(giocatore.x,giocatore.y)== 'C')
        {
            giocatore.checkPoint+=1;
            for(int i=0; i<=20; i++)
            {
                if ((giocatore.x==zbierajka[i].x) && (giocatore.y==zbierajka[i].y))
                {
        ...
        if (readChar(giocatore.x,giocatore.y)== 'S')
        {
            giocatore.vite-=1;
            giocatore.ucciso=true;
        }
        if (readChar(giocatore.x,giocatore.y)== '%')
        {
            giocatore.vite-=1;
        ...
        if (readChar(giocatore.x,giocatore.y)== 'o')
        {
            giocatore.vite-=1;
            giocatore.ucciso=true;
        }
        ...
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),11);
        gotoxy(giocatore.x,giocatore.y);
        if (giocatore.pagina=='w')
        {
            cout<<char(24);
        }
        if (giocatore.pagina=='s')
        {
            cout<<char(25);
        }
        if (giocatore.pagina=='a')
        {
            cout<<char(27);
        }
        if (giocatore.pagina=='d')
        {
            cout<<char(26);
        }
    }
    ...
    for (int i=0; i<=30; i++)
    {
        ...
        {
            SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),9);
            if (abs(scigacz[i].x-giocatore.x)>=(abs(scigacz[i].y-giocatore.y)))
            {
                if(scigacz[i].x>=giocatore.x)
                {
                ...
                }else if(scigacz[i].x<giocatore.x)
                {
                    if(readChar(scigacz[i].x+1,scigacz[i].y)==' ')
                    {
                        gotoxy(scigacz[i].x,scigacz[i].y);
            ...
            }else if (abs(scigacz[i].x-giocatore.x)<(abs(scigacz[i].y-giocatore.y)))
            {
                if(scigacz[i].y>=giocatore.y)
                {
                ...
                }else if(scigacz[i].y<giocatore.y)
                {
                    if(readChar(scigacz[i].x,scigacz[i].y+1)==' ')
                    {
                        gotoxy(scigacz[i].x,scigacz[i].y);
    ...
    for (int i=0;i<=100;i++)
    {
        if (biegaczPoz[i].vite>0)
        {
            gotoxy(biegaczPoz[i].x,biegaczPoz[i].y);
            cout<<" ";
            if (biegaczPoz[i].pagina=='d')
            {
    ...
    for (int i=0; i<=50; i++)
    {
        if (armataPio[i].istnieje==true)
        {
            ...
            {
                if (armataPio[i].pagina=='w')
                {
                    nrPocisk+=1;
                    pocisk[nrPocisk].istnieje=true;
                    pocisk[nrPocisk].x=armataPio[i].x;
                    pocisk[nrPocisk].y=armataPio[i].y;
                    pocisk[nrPocisk].pagina=armataPio[i].pagina;
                    pocisk[nrPocisk].pierwszyRaz=true;
                }
                if (armataPio[i].pagina=='s')
                {
                    nrPocisk+=1;
                    pocisk[nrPocisk].istnieje=true;
                    pocisk[nrPocisk].x=armataPio[i].x;
    ...
    if (c=='m')
    {
        if (giocatore.munizioni!=0)
        {
            nrPocisk+=1;
            pocisk[nrPocisk].istnieje=false;
            pocisk[nrPocisk].pierwszyRaz=true;
            giocatore.munizioni-=1;
            if (giocatore.pagina=='w')
            {
                pocisk[nrPocisk].x=giocatore.x;
                pocisk[nrPocisk].y=giocatore.y;
                pocisk[nrPocisk].pagina='w';
                if ((readChar(giocatore.x,giocatore.y-1)==' ') || (readChar(giocatore.x,giocatore.y-1)=='S') || (readChar(giocatore.x,giocatore.y-1)=='%')|| (readChar(giocatore.x,giocatore.y-1)=='X'))
                {
                    pocisk[nrPocisk].istnieje=true;
                }
            }
            ...
            if (giocatore.pagina=='a')
            {
                pocisk[nrPocisk].x=giocatore.x;
                pocisk[nrPocisk].y=giocatore.y;
                pocisk[nrPocisk].pagina='a';
                if ((readChar(giocatore.x-1,giocatore.y)==' ') || (readChar(giocatore.x-1,giocatore.y)=='S') || (readChar(giocatore.x-1,giocatore.y)=='%')|| (readChar(giocatore.x-1,giocatore.y)=='X'))
                {
                    pocisk[nrPocisk].istnieje=true;
                }
            }
    ...
    cout<<"Lives: "<<giocatore.vite;

    gotoxy(90,5);
    cout<<"                ";
    gotoxy(90,5);
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),14);
    cout<<"Ammo: "<<giocatore.munizioni;

    gotoxy(90,7);
    cout<<"                           ";
    if (giocatore.tempo_respawn>0)
    {
        gotoxy(90,7);
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),15);
        cout<<"Respawn time: "<<giocatore.tempo_respawn;
    }
    ...
    gotoxy(11,40);
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),11);
    cout<<"XY: ( "<<giocatore.x<<", "<<giocatore.y<<" )";
...
void restartPoziomu()
{
    ...
    licznikPrzejsc=0;
    system("CLS");
    if(giocatore.checkPoint==0)
    {
        giocatore.x=2;
        giocatore.y=35;
    }
    if (giocatore.checkPoint==1)
    {
        giocatore.x=56;
        giocatore.y=22;
    }
    if (giocatore.checkPoint==2)
    {
        giocatore.x=4;
        giocatore.y=23;
    }
    if (giocatore.checkPoint==3)
    {
        giocatore.x=33;
        giocatore.y=11;
    }
...
void rozpocznijGre()
{
    ...
    giocatore.x=2;
    giocatore.y=35;
    giocatore.munizioni=15;
    giocatore.vite=3;
    giocatore.checkPoint=0;
    bossZycie=50;
...
int main()
{
    ...
    while (wyjscie==false)
    {
        ...
        while(wGrze==true)
        {
            giocatore.ucciso=false;
            aktualizuj();
            poruszenieGiocatore();
            if (giocatore.tempo_respawn>=1)
            {
                c='0';
                giocatore.tempo_respawn-=1;
            }
            if (c==char(27))
            {
                wGrze=false;
            }
            if ((giocatore.ucciso==true) || (c=='r'))
            {
                for (int i=0; i<=4; i++)
                {
                    gotoxy(giocatore.x,giocatore.y);
